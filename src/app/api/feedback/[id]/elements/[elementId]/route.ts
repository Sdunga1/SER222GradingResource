...
export async function PUT(
  ...
  try {
    ...
    updateQuery += ` WHERE id = $${paramIndex} RETURNING id, module_id, content, position, created_at, updated_at`;
    params_arr.push(elementId);

    const { rows } = await query(updateQuery, params_arr);

    if (!rows[0]) {
      return NextResponse.json({ success: false, message: 'Element not found' }, { status: 404 });
    }
...
// PATCH update a feedback element
...
export async function DELETE(
  ...
  try {
    ...
    const { rowCount } = await query(`DELETE FROM feedback_elements WHERE id = $1`, [elementId]);
